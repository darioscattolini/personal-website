...
import OverflowImage from '@site/src/components/shared/OverflowImage/OverflowImage';
...
### 1. Clase madre con miembros estáticos
...
### 2. Interfaz de instancia con miembros estáticos
...
representadas. Esa interfaz representaría a las *instancias* de estas clases,
pero podríamos intentar incluir en ella nuestra interfaz estática. El resultado
quedaría así:
...
### 3. Expresiones de clase tipadas con la interfaz estática
...
pero tiene una serie de limitaciones. En primer lugar, veamos lo que ocurriría
**si las clases representadas tuvieran que ser abstractas**. Primero deberíamos
modificar el distintivo de constructor en `MonsterStatic`, ya que las clases
abstractas no pueden crear instancias con `new`. Esto se puede hacer porque
[recientemente TypeScript introdujo la posibilidad de **especificar distintivos
de constructor
...
type MonsterStatic = { maxAmount: number; } & AbstractConstructor<Monster>;
```
...
### 4. Interfaz estática asignada mediante `InstanceType`

La solución que mostraremos ahora es menos intuitiva que las anteriores, ya que
explota las posibilidades ofrecidas por TypeScript de una forma bastante más
artificiosa. La clave de esta solución es retomar la estrategia de nuestro
...
interfaces de instancia**. Ya no necesitamos reservarnos `Monster` para el alias
de `InstanceType`, por lo que `MonsterInstance` puede volver a llamarse
`Monster`.

Veamos ahora si algunas variantes de la solución anterior también pueden
aplicarse a esta:

* **`Monster` puede ser una clase madre** en lugar de una mera interfaz con
  mínimas modificaciones en el código para implementar la herencia.
* **Las clases representadas también pueden ser abstractas**. Sólo hace falta
  redefinir el distintivo de constructor de la misma forma que en la sección
  anterior para admitir constructores abstractos.
* Sin embargo, **no es posible sustituir el constructor por un método 
  *factory** estático* como `create` para instanciar monstruos: el tipo
  `InstanceType` requiere como argumento un tipo que incluya un distintivo de
  constructor.

En consecuencia, se trata de una solución potente que cumple con todos nuestros
requisitos. Sin embargo, **las definiciones de las clases representadas se
vuelven demasiado extensas y rebuscadas**. Alguien que vea a simple vista un
código escrito de esa forma seguramente no entendería a primera vista el porqué
de estos constructos verbosos y artificiosos, lo que requeriría comentar el
código de forma clara. Es por ello que en la siguiente sección ofrecemos otra
solución más concisa y elegante.

### 5. Interfaz estática asignada mediante un decorador

Hay una funcionalidad cuya inclusión en JavaScript está en discusión y en
TypeScript es de carácter experimental: se trata de [**los *decoradores*, que
...
que se aplican y la transforman de alguna manera. Como hemos dicho, son una
sintaxis experimental que puede estar sujeta cambios, probablemente en función
de cómo se termine de implementar en JavaScript.

Veremos ahora que los decoradores pueden ser útiles para construir otra solución
nuestro problema. En nuestro caso **no queremos realizar ninguna transformación
de las clases representadas, sino lograr que TypeScript fuerce que implementen
una interfaz estática**. Esto implica dos cosas:

* La función que usemos como decorador **no debe devolver nada**, ya que si no 
  tiene un `return` devuelve la misma clase que recibe como argumento. 
* Es necesario **tipar el parámetro de la función con la interfaz estática que 
  se desea requerir**. Si el parámetro de la función debe satisfacer la interfaz 
  `MonsterStatic`, al usarla como un decorador TypeScript requerirá que la clase
  implemente dicha interfaz. 

Nuestro caso de uso entonces se vería así, utilizando las versiones canónicas de
`Monster` y `MonsterStatic` definidas anteriormente:
...
// decorador
function MonsterClass<T extends AttackType>(monsterClass: MonsterStatic<T>) { }
...
Nótese que ahora debemos incluir los paréntesis al invocar la función *factory*,
ya que necesitamos que esta se ejecute para obtener el decorador.

En cuanto a su capacidad de replicar las variantes de las soluciones anteriores,
esta solución es óptima:

* **`Monster` puede ser una clase madre**.
* **Las clases representadas también pueden ser abstractas** si el distintivo de
  constructor en `MonsterStatic` es abstracto.
* Para la instanciación de monstruos **es posible sustituir el constructor por
  un método *factory* estático**.

Esta solución es concisa y elegante, pero **depende de una sintaxis experimental
que podría quedar obsoleta** ante una actualización de TypeScript. Para poder
usarla es necesario **habilitar la opción `experimentalDecorators: true` en
`tsconfig.json`**.

## Un caso de uso más realista

El caso de uso que hemos utilizado hasta ahora nos ha facilitado presentar el
problema de una forma simple e ir complejizándolo con nuevos requerimientos a
medida que íbamos presentando las distintas soluciones. Sin embargo, no me
parece un ejemplo demasiado realista. En lugar de regular la creación de
instancias de monstruos mediante propiedades estáticas contenidas en sus propias
clases, lo más apropiado sería que haya una clase separada que se encargue de
poblar los diferentes escenarios del juego a partir de los datos de
configuración que se le provean. Una interfaz como `MonsterStatic` no tiene una
verdadera razón de ser.

Para terminar este artículo veremos finalmente un caso de uso más realista.
Supongamos que estamos desarrollando una tienda online que debe mostrar listas
ordenadas de diferentes entidades. Por ejemplo, rankings de clientes, empleados
y productos. Estas entidades serán representadas en la aplicación mediante las
clases `Customer`, `Employee` y `Article`. Cada una de estas clases puede
admitir un método de ordenamiento diferente para sus respectivas instancias: los
clientes serán ordenados en función de la cantidad de productos comprados, los
empleados en función de su antigüedad y los artículos en función de su
popularidad.
...
También es necesario incluir dentro de `Sorting` un distintivo de constructor.
Esto permitirá que TypeScript asocie el parámetro `T` con el tipo de las
instancias de cada clase que implementer `Sorting`. De lo contrario el método
`sort` de cada clase podría admitir como parámetro cualquier tipo de valor,
usándose por ejemplo `Article.sort` para ordenar números o instacias de
`Customer`. La interfaz resultante es:
...
Dado que `Sorting` ya incluye un distintivo de constructor, puede ser utilizado
directamente como argumento de `InstanceType`. El tipo que debe implementar cada
una de nuestras clases será entonces el siguiente:

```ts
type Sortable<T, S extends Sorting<T>> = InstanceType<S>;
```

El parámetro `T` en `Sortable` representa el tipo de entidad que es instanciada
por el constructor y ordenada por el método estático `sort`.

Ahora podemos construir nuestras clases representadas de la siguiente manera:

```ts
class Customer implements Sortable<Customer, typeof Customer>{
  constructor(
    public name: string,
    public email: string,
...
class Employee implements Sortable<Employee, typeof Employee>{
  constructor(
    public name: string,
...
class Article implements Sortable<Article, typeof Article>{
  constructor(
    public name: string,
    public ean: string,
    public purchases: number
...
construir un componente `SortedList` para que nuestros rankings se muestren como
un elemento HTML de tipo `<ol>`. Esto muestra la capacidad de manipular
programáticamente los tipos antes desarrollados. El método `render` del
componente devuelve una lista HTML con la cantidad de elementos especificados en
su argumento. Aquí optamos por construir el componente genérico como una clase
abstracta y usar distintas clases hijas para cada tipo de lista:
...
Para esta segunda solución definiremos el siguiente decorador:

```ts
function SortingClass<T>(sortingClass: Sorting<T>) { }
```

Luego podemos construir las clases representadas de la siguiente manera:
...
El componente `SortedList` y sus diferentes clases derivadas pueden ser
construidos exactamene igual a como lo hicimos en la solución anterior.

## Referencias

Este tópico ha sido discutido extensamente en el repositorio de TypeScript ante
los reiterados pedidos de incluir en el lenguaje una sintaxis más sencilla para
este tipo de casos de uso. Allí se pueden encontrar diversos ejemplos que
requieren el uso de interfaces estáticas, soluciones alternativas dentro de las
posibilidades que ofrece TypeScript hoy, y propuestas para incorporar nuevas
funcionalidades a TypeScript que permitan trabajar con estas interfaces de forma
más clara y directa:
