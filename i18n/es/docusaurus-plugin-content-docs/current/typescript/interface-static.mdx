...
variables y ser representados por tipos en TypeScript. Para comprender qué forma
tiene el tipo de una clase hay que analizar el tipo de objeto que son las clases
en JavaScript. Podemos observarlo en el resultado de transpilar una clase de
TypeScript a EcmaScript 5:
...
constructora](https://javascript.info/constructor-new) como `Example`. Sus
...
objeto `prototype` de la función constructora. La función constructora es un
objeto, y por ende se le pueden añadir nuevas propiedades. Las propiedades y
métodos estáticos de la clase no son otra cosa que propiedades añadidas a la
función constructora en tanto objeto. En consecuencia, lo que antes hemos
definido como una interfaz estática es una interfaz que representa estas
propiedades adicionales de la función constructora (y opcionalmente el tipo de
la función constructora misma).
...
utilidad. Sin embargo, también podemos construir explícitamente ese tipo:
debemos tipar la función constructora y añadir los tipos para las propiedades
estáticas. Más adelante veremos que nuestras interfaces estáticas tendrán esta
forma:
